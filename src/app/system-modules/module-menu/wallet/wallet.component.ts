...
export class WalletComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.paymentChannels = PaymentChannels.filter(
      x => x.name === 'Flutterwave' || x.name === 'Paystack'
    );
    // this.refKey =
    ...
    this.paymentFormGroup.controls['paymentType'].valueChanges.subscribe(
      val => {
  ...
  paymentDone(paymentRes) {
    ...
    const desc =
      'Funded wallet via ' +
      this.paymentFormGroup.controls['paymentType'].value;
