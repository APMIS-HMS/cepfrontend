...
export class FcListComponent implements OnInit {
	...
	pushNewDependant(dependant?, index?) {
		...
		(<FormArray>this.frmDependant.controls['dependantArray']).push(
			this.formBuilder.group({
				surname: [ '', [ Validators.required ] ],
				othernames: [ '', [ Validators.required ] ],
				gender: [ '', [ Validators.required ] ],
	...
	save(valid, value, dependantValid, dependantValue) {
		...
		const unsavedFiltered = dependantValue.controls.dependantArray.controls.filter(
			(x) => x.value.readOnly === false && x.valid
		);
		if (unsavedFiltered.length > 0) {
			this.loading = false;
			this.updatePatientBtnText = 'Add Family';
			this.systemModuleService.announceSweetProxy(
				'There seems to unsaved but valid dependant yet to be saved, please save and try again!',
				'warning'
			);
			return;
		}
		if (valid) {
			...
			this.familyCoverService.updateBeneficiaryList(param).then(
				(payload) => {
					this.loading = false;
					this.updatePatientBtnText = 'Add Family';
					this.getBeneficiaryList(this.selectedFacility._id);
					this.cancel();
					this.systemModuleService.announceSweetProxy(
						'Family Cover Records Updated Successfully',
						'success',
						null,
						null,
						null,
			...
			this.updatePatientBtnText = 'Add Family';
			this.systemModuleService.announceSweetProxy(
				'A value is missing, please fill all required field and try again!',
				'warning'
			);
		}
	}
