...
import { FacilitiesService, LaboratoryRequestService, InvestigationService, } from '../../../../services/facility-manager/setup/index';
...
export class ExternalInvestigationsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.patientSearch.valueChanges
      ...
      .do(val => { this.extRequests = []; this.loading = true; })
          ...
          res.data.forEach(element => {
            element.investigations.forEach(item => {
              if (item.isExternal) {
                ...
                const checkDuplicate = this.allInvestigationWithExternal.filter(x => x._id === element._id);
  ...
  onClickSaveCost() {
    if (!!this.selectedLabRequest._id) {
      this._laboratoryRequestService.patch(this.selectedLabRequest._id, this.selectedLabRequest, {}).then(res => {
        const text = 'You have successfully attended to an external request. You can now write report for the investigation';
  ...
  locationChanged($event, investigation: InvestigationModel, location, LaboratoryWorkbenches) {
    ...
    // }
    const addLocation = this.allInvestigationWithExternal.filter(x => x._id === this.selectedInvestigation.labRequestId);
    if (addLocation.length > 0) {
      // attach the laboratoryWorkbenches to the selected investigation
      const investigationToAttachLocation = addLocation[0].investigations.filter(x => x.investigation._id === this.selectedInvestigation.investigationId);
