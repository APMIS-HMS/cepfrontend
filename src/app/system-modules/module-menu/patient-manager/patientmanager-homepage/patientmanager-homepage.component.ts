...
export class PatientmanagerHomepageComponent implements OnInit, OnChanges {
	...
	getRelationships() {
		this.relationshipService.find({}).then(
			(payload) => {
	...
	ngOnInit() {
		...
		this.patientEditForm.controls['country'].valueChanges.subscribe((val) => {
			this.countryFacadeService
				.getOnlyStates(val, true)
				.then((states: any) => {
					this.states = states;
				})
				.catch((err) => {});
		...
		this.patientEditForm.controls['state'].valueChanges.subscribe((val) => {
			...
			this.countryFacadeService
				.getOnlyLGAndCities(country, val, true)
				.then((lgsAndCities: any) => {
					this.cities = lgsAndCities.cities;
	...
	removeTag(tag) {
		...
		if (toDelete > -1) {
			...
			this.patientService
				.patch(this.patientToEdit._id, this.patientToEdit, {})
				.then((deletePayload) => {})
				.catch((err) => {});
		}
	}
	...
	navEpDetail(patient) {
		...
		this.router
			.navigate([ '/dashboard/patient-manager/patient-manager-detail', patient.personId ])
			.then(() => {
	...
	updatePatient(value: any, valid: boolean) {
		...
		this.selectedPatient['fullAddress'] =
			value.street + ', ' + value.city.name + ', ' + value.state.name + ', ' + value.country.name;
		...
		this.personService
			.patch(this.selectedPatient._id, this.selectedPatient, {})
			.then((res) => {
			...
			.catch((err) => {
				...
				this.systemService.announceSweetProxy(
					'There was an error updating user record, Please try again later.',
					'Error'
				);
			});
	}
							...
							if (facHmo.hmos[index].enrolleeList.length > 0) {
								...
								if (fil.length > 0) {
									if (fil[0].status.toLowerCase() !== 'active') {
										...
										if (
											fil[0].firstname !== this.patient.personDetails.firstName ||
											fil[0].surname !== this.patient.personDetails.lastName
										) {
											this.systemService.off();
											this.loading = false;
											const text =
												"Information of policy doesn't match patient details. Please check and try again! ";
											this.errMsg = text;
											this.mainErr = false;
											return false;
											// this.systemService
						...
						if (companyCover.companyCovers[index].enrolleeList.length > 0) {
							...
							if (fil.length > 0) {
								if (fil[0].status.toLowerCase() !== 'active') {
									...
									this.errMsg = text;
									this.mainErr = false;
									this.systemService.announceSweetProxy(text, 'error');
								} else {
									if (
										fil[0].firstname !== this.patient.personDetails.firstName ||
										fil[0].surname !== this.patient.personDetails.lastName
										...
										this.systemService.off();
										const text =
											'Employee Id does not have an active status for the selected Company';
										this.errMsg = text;
										this.mainErr = false;
										this.systemService.announceSweetProxy(text, 'error');
									} else {
								...
								if (filEx[0].patientId !== undefined) {
									...
									if (info[0].patientId === undefined) {
										if (this.getRole(this.familyClientId) !== 'P') {
											...
											this.loading = false;
											const text =
												"Principal doesn't exist as a patient. Please register principal.";
											this.errMsg = text;
											this.mainErr = false;
