...
import {Appointment, BillIGroup, BillItem, Dispensed, Employee, Facility, Prescription, PrescriptionItem, User} from '../../../../../models/index';
import {BillingService, DictionariesService, DrugDetailsService, DrugListApiService, FacilitiesService, FrequencyService, MedicationListService, PersonService, PrescriptionPriorityService, PrescriptionService, RouteService} from '../../../../../services/facility-manager/setup/index';
import {DosageUnits, DurationUnits} from '../../../../../shared-module/helpers/global-config';
...
export class PatientPrescriptionComponent implements OnInit {
  ...
  constructor(
      private fb: FormBuilder, private _locker: CoolLocalStorage,
  ...
  ngOnInit() {
    ...
    this._authFacadeService.getLogingEmployee()
        .then(res => {
  ...
  onClickAuthorizePrescription(value: any, valid: boolean) {
    if (valid && (this.prescriptionArray.length > 0)) {
      ...
      this.prescriptions.priority = {
        id: value.priority._id,
      ...
      this._prescriptionService.authorizePresciption(this.prescriptions)
          .then(res => {
            console.log(res);
            if (res.status === 'success') {
              this._systemModuleService.announceSweetProxy(
                  'Prescription has been sent successfully!', 'success');
