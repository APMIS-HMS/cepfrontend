...
import {Document, Documentation, Employee, Facility, Patient, PatientDocumentation} from '../../../../../../models/index';
import {DocumentationService, FacilitiesService, FormsService} from '../../../../../../services/facility-manager/setup/index';
...
export class ClinicalNoteComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.authFacadeService.getLogingEmployee()
        .then((payload: any) => {
    ...
    this.subscription =
        this.sharedService.surveyInitialized$.takeUntil(this.onDestroy$)
            .subscribe((value: any) => {
              if (this.formJsonObject !== undefined &&
                  this.formJsonObject !== null) {
              ...
              if (this.isEditingPatient) {
                ...
                this.sharedService.announceEditDocumentation(
                    {document: this.editingDocumentation, leg: 1});
              } else {
  ...
  getForms() {
    try {
      ...
      formType$
          .mergeMap(
              (formTypes: any) => Observable.fromPromise(this.formService.find({
                query: {
                  $or: [
  ...
  checkIfSurveyIsAvailable() {
    this.surveySubscription =
        Observable.interval(1).takeUntil(this.onDestroy$).subscribe(x => {
  ...
  setSelectedForm(form) {
    if (typeof form === 'object' && form !== null) {
      ...
      this.systemModuleService.announceSweetProxy(
          'wow', 'success', null, null, null, null, null, null, null);
    }
  }
  ...
  addSymptom(item) {
    ...
    this.sharedService.announceDiagnosisSystemOrder(
        {type: 'Symptoms', action: 'add', data: item});
  }

  deleteSymptom(item) {
    this.symptoms = this.symptoms.filter(e => e !== item);
    this.sharedService.announceDiagnosisSystemOrder(
        {type: 'Symptoms', action: 'remove', data: item});
  }
  deleteDiagnosis(item) {
    this.diagnoses = this.diagnoses.filter(e => e !== item);
    this.sharedService.announceDiagnosisSystemOrder(
        {type: 'Diagnoses', action: 'remove', data: item});
  }
  ...
  addDiagnosis(item) {
    ...
    this.sharedService.announceDiagnosisSystemOrder(
        {type: 'Diagnoses', action: 'add', data: item});
  }
