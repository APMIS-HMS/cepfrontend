...
export class NewSupplierComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.frm_newSupplier = this.formBuilder.group({
      ...
      frmContact: ['', [<any>Validators.required, <any>Validators.minLength(10), <any>Validators.pattern('^[0-9]+$')]],
  ...
  _getCountries() {
    this._countryServiceFacade.getOnlyCountries().then((res: any) => {
      if (res.length > 0) {
        this.countries = res;
      }
    }).catch(error => {});
  ...
  create(valid, value) {
    if (valid) {
      ...
      this.supplierService.customCreate(payload).then(res => {
