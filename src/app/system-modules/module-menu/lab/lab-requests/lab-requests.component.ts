...
export class LabRequestsComponent implements OnInit {
  ...
  apmisInvestigationLookupHandleSelectedItem(value) {
    if (value.action !== undefined) {
      ...
      if (value.checked === true) {
        if (
          this.checkedValues.filter(item => item.name === value.object.name)
            .length === 0
        ) {
        ...
        if (
          ...
          const index = this.checkedValues.findIndex(
            item => item.name === value.object.name
          );
            ...
            if (isInBind > -1) {
              if ($event.checked) {
                if (
                  ...
                  if (
                    this.bindInvestigations[isInBind].investigation.panel
                      .length === investigation.investigation.panel.length
                  ) {
                    investigation.isChecked = true;
                  } else {
                    investigation.isChecked = false;
                ...
                const indexToRemove = this.bindInvestigations[
                  isInBind
                ].investigation.panel.findIndex(
                  x =>
  ...
  locationChanged(
    ...
    if (investigation.investigation.isPanel) {
      ...
      copyBindInvestigation.investigation.LaboratoryWorkbenches =
        copyBindInvestigation.LaboratoryWorkbenches;
  ...
  getChildPrice(investigation, panel) {
    ...
    if (investigation.temporaryInvestigationList !== undefined) {
      investigation.temporaryInvestigationList.forEach(item => {
        if (
          item.laboratoryId._id === parentLocation._id &&
          item.investigationId === panel.investigation._id
        ) {
  ...
  IsParentChecked(investigation, panel) {
    return (
      this.bindInvestigations.findIndex(
        x => x.investigation._id === investigation.investigation._id
      ) > -1 || investigation.isChecked
    );
  }
  ...
  save(valid, value) {
    ...
    const request: any = {
      facilityId: this.selectedFacility._id,
      patientId: this.isLaboratory
        ? this.selectedPatient.patientId
        : this.selectedPatient._id,
      labNumber: !this.isLaboratory
        ? this.frmNewRequest.controls["labNo"].value
        : "",
