...
// TODO :  This section requires thorough check, especially when res.wallet ={ ...default} ;
        if (!!this.patient.personId) {
            this.personService.get(this.patient.personId, {}).then(res => {
                ...
                if (res.wallet === undefined) {
                    ...
                    console.log("BEFORE Update was Called, value of res variable", res);
                    this.personService.update(res).then(pay => {
                        this.person = pay;
                        console.log("AFTER Update was Called, value of pay variable", pay);
                    });
                } else {
                    this.person = res;
                    console.log("Transactions before Slice", res.wallet.transactions);
                    this.transactions = res.wallet.transactions.reverse().slice(0, 5);
                    console.log("Transactions After Slice", this.transactions);
                }
            });
        }
    ...
    fundWalletViaCashPayment(valid: boolean, value: any) {
        if (valid && parseFloat(value.fundAmount) >= 500) {
            ...
            console.log("wallet Transaction info before calling endpoint",walletTransaction, this.selectedFacility._id);
            this.personService.fundWallet(walletTransaction, this.selectedFacility._id)
                .then((res: any) => {
                    if (res.status === 'success') {
                        ...
                        console.log("BEFORE Reverse and Slice function is called on this.transaction",this.transactions);
                        this.transactions = this.person.wallet.transactions.reverse().slice(0, 10);
                        console.log("Reverse and Slice function called on this.transaction with slice(0,10)",this.transactions)
