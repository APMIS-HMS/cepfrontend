...
export class PaymentHistoryDetailsComponent implements OnInit {
      ...
      if (id !== undefined) {
        this.invoiceService.get(id, {}).then(payload => {
          ...
          this.invoiceDetails.payments.forEach(element => {
            try {
              this.employeeService.get(element.createdBy, {}).then(payload => {
  ...
  getUnitPrice(serviceId) {
    const unitPrice = this.invoiceDetails.billingIds.find(x => x.billObject.facilityServiceObject.serviceId.toString() === serviceId.toString());
