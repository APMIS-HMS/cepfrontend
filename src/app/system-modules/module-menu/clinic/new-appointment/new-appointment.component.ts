...
import {Appointment, AppointmentType, ClinicModel, Employee, Facility, Profession, ScheduleRecordModel} from '../../../../models/index';
// var _ = require('lodash');

import {AppointmentService, AppointmentTypeService, EmployeeService, FacilitiesService, PatientService, ProfessionService, SchedulerService, WorkSpaceService} from '../../../../services/facility-manager/setup/index';
...
export class NewAppointmentComponent implements OnInit {
  ...
  dayCount =
      ['Today', 'Last 3 Days', 'Last Week', 'Last 2 Weeks', 'Last Month'];

  constructor(
      private _fb: FormBuilder, private scheduleService: SchedulerService,
        ...
        this.appointmentService.getAppointment(params.id, {})
            .subscribe(payload => {
              ...
              if (payload.immunizationRecords !== undefined) {
                this.appointmentTypeForm.controls['category'].setValue(
          ...
          params.patientId !== undefined && params.doctorId !== undefined) {
    ...
    this.filteredClinics =
        this.clinicCtrl.valueChanges.startWith(null)
            .map(
                clinic => clinic && typeof clinic === 'object' ?
                    clinic.clinicName :
                    clinic)
    ...
    this.filteredProviders =
        this.providerCtrl.valueChanges.startWith(null)
            .map(
                (provider: Employee) =>
                    provider && typeof provider === 'object' ?
                    provider.employeeDetails.lastName :
                    provider)
            .map(
                val =>
                    (val ? this.filterProviders(val) : this.providers.slice()));


    this.typeCtrl = new FormControl();
    this.filteredAppointmentTypes =
        this.typeCtrl.valueChanges.startWith(null)
            .map(
                (type: AppointmentType) =>
                    type && typeof type === 'object' ? type.name : type)
            .map(
                val => val ? this.filterAppointmentTypes(val) :
  ...
  ngOnInit() {
    ...
    this.appointmentTypeForm =
        this._fb.group({category: ['physicianAppointment']});
  }
  ...
  getPreviousAppointments(value) {
    ...
    if (value._id !== undefined) {
      this.appointmentService.findAppointment({query: {patientId: value._id}})
          .subscribe(payload => {
              ...
              wrk.locations.forEach((lct, li) => {
                this.gSchedules.forEach((sch: any, ji) => {
                  sch.schedules.forEach((sch2, jji) => {
                    if (sch2.location._id === lct.minorLocationId._id &&
  ...
  prime() {
    if (this.loginEmployee._id !== undefined) {
      ...
      this.subscription =
          Observable
              .forkJoin([
                Observable.fromPromise(this.workSpaceService.find(
                    {query: {'employeeId._id': this.loginEmployee._id}})),
                Observable.fromPromise(this.appointmentTypeService.findAll()),
                Observable.fromPromise(this.professionService.findAll()),
                Observable.fromPromise(this.scheduleService.find(
              ...
              .subscribe((results: any) => {
                ...
                const filteredProfessions =
                    professions.filter(x => x.name === 'Doctor');
  ...
  filterProviders(val: any) {
    return val ? this.providers.filter(
                     s => s.employeeDetails.lastName.toLowerCase().indexOf(
                              val.toLowerCase()) === 0 ||
                         s.employeeDetails.firstName.toLowerCase().indexOf(
                             val.toLowerCase()) === 0) :
                 this.providers;
  }
  ...
  providerDisplayFn(provider: any): string {
    return provider ? provider.employeeDetails.lastName + ' ' +
            provider.employeeDetails.lastName :
                      provider;
  }
