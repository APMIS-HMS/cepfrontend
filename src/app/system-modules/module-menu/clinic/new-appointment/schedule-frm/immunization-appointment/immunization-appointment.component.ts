...
export class ImmunizationAppointmentComponent implements OnInit {
	...
	ngOnInit() {
		if (this.selectedAppointment !== undefined) {
			...
			if (
				!!this.selectedAppointment.immunizationRecords &&
				this.selectedAppointment.immunizationRecords.length > 0
			) {
	...
	scheduleAppointment() {
		if (this.dateCtrl.valid && this.patient.valid && this.type.valid && this.category.valid && this.clinic.valid) {
			...
			if (this.appointment._id !== undefined) {
				...
				this.appointmentService.updateImmunizationAppointment(vaccineAppointments).then(
					(payload) => {
						this.savingAppointment = false;
						this.disableBtn = false;
						this.loadIndicatorVisible = false;
				...
				this.appointmentService.setMultipleAppointments(vaccineAppointments).then(
					(payload) => {
						this.savingAppointment = false;
						this.disableBtn = false;
						this.loadIndicatorVisible = false;
