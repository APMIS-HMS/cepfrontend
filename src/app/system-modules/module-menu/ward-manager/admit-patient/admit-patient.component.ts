    ...
    if (valid) {
      ...
      if (this.inPatientItem.typeChecker === myGlobals.onAdmission) {
        ...
        let payload = {
          ...
          statusId: myGlobals.onAdmission,
          ward: value.ward,
          room: value.room,
          bed: value.bed,
          desc: value.desc,
        ...
        this._inPatientService
          ...
          .then(res => {
            ...
            if (res.status === 'success') {
              this._notification(
                'Success',
                'Patient has been accepted successfully.'
              );
              setTimeout(e => {
                this.router.navigate(['/dashboard/ward-manager/admitted']);
              }, 2000);
              ...
              this._notification(
                'Error',
                'There was a problem discharging patient. Please try again later.'
              );
            }
          })
          .catch(err => {
