...
export class DashboardHomeComponent implements OnInit {
  ...
  getFacilitySubscription() {
    this.facilityService
        .findValidSubscription({query: {facilityId: this.facilityObj._id}})
        .then(payload => {
          this.facilitySubscriptions = payload.data;
          this.facilitySubscriptions.subscriptions_status =
              payload.data.subscriptions_status;
        });
  }
  ...
  accessHas(menu) {
    ...
    if (modules !== undefined) {
      const index =
          modules.findIndex(x => x.route.substring(1) === menu.toLowerCase());
